...
Максимально постарался сделать без js, но в итоге js используется для проверки поддерживается ли
position: sticky в браузере. И делает свой sticky, если не поддерживается. Position sticky поддерживается на данный момент только в firefox
и в safari с префиксом -webkit-.